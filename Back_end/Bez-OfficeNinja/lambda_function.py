...
PATH_MAP = {
    "/secure_chat": {"POST": "bez_securegeneral_llm._secure_chat"},
    "/llm_list":{"GET":"bez_llm_list._list_of_llms",
                "POST": "bez_llm_list._update_llm"}}
...
def _get_path_handler(path, method, workflow, event):
    ...
    if path:
        module = import_module(f"bez_llm_modules.{module_path}")
    # elif workflow:
